...
(def reg-raw
  '{ns          {:core-exclusions {:cljc #{defonce}}
                 ...
                 :aliases         {:cljc {ns      quantum.core.ns  
                                   ...
                                   :cljs {core cljs.core}}
                 :refers          {:cljc
                                    {ns   #{defalias defonce source def- ns-exclude js-println
    ...
    cmacros     {:aliases         {:cljc {cmacros     quantum.core.macros.core}}
                 :refers          {:cljc {cmacros     #{if-cljs resolve-local}}}}
...
(def reg
  (let [lib-exclusions (set/union '#{red loops ccore} '#{http web auth url ui}) ; Because contained in coll
        ...
        lib
          {:core-exclusions (->> lib-keys (get-ns-syms reg-raw :core-exclusions) #_(into #{}))
...
(def ^{:doc "All macros in Quantum library. Primarily for ClojureScript's :refer-macros clause."}
 ...
 '{quantum.core.cljs.macros  #{defnt}
   ...
   quantum.core.ns           #{def- defalias defonce ns-exclude source},
   quantum.core.numeric      #{+= -=
                               ++ --}
   ...
   quantum.core.macros.core  #{if-cljs resolve-local}})