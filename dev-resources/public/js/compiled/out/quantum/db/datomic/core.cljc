(ns ^{:doc "The core Datomic (and friends, e.g. DataScript) namespace"}
  ...
  (:refer-clojure :exclude [assoc dissoc conj disj update async])
  (:require-quantum [ns err core-async pr log logic fn cbase tpred])
  (:require
...
(defn block->schemas
  ...
  [block]
  (->> block
       (mapv (fn [[ident args]] (apply ->schema ident args)))))
  ...
  ([conn schemas]
    (if (mconn? conn)
        (let [schemas-f (->> schemas
                             ...
                             (into {}))]
          (swap! conn c/update :schema merge schemas-f)) ; TODO there hopefully is a better way...? 
        (transact! schemas))))
